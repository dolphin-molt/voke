...
核心输入链路和主要回归问题已经解决，并有 31 项单元测试覆盖关键逻辑。项目现在处于“功能主干完成，正式产品化未完成”的阶段。
...
- 主界面不再显示 Profile 增删切换。旧 Profile 数据仍保留在 v2 备份结构中，避免无声丢失用户配置。
...
- 主界面每台设备只展示一套配置；底层 v2 数据结构暂时保留 Profile 容器以兼容旧数据，但 UI 不再提供多方案管理。
...
当前共有 31 项 XCTest，最近一次实跑全部通过。除原有快捷键、摇杆、HID 和备份测试外，现已覆盖：
